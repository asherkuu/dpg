...
public class SHA256 {
	...
	public static String encrypt(String planText) {
		...
		try {
			/*
			 * 1. SHA-256으로 해시 - MessageDigest객체 생성시 알고리즘을 "SHA-256"으로 해서 만든다. 해시된 데이터는 바이트
			 * 배열타입의 바이너리 데이터(이진 데이터)이다.
			 */
			...
			/*
			 * 2. 해시된 데이터는 바이트 배열타입의 바이너리 데이터(이진 데이터)이므로 이것을 16진수 문자열(String)타입으로 변환해준다.
			 */
